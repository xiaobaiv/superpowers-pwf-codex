...
| `UserPromptSubmit` | 每次用户消息前注入 `task_plan.md` 前 50 行和 `progress.md` 后 20 行 |
...
- `UserPromptSubmit` 采用 PWF 原版思路：只要有 active task，每次用户消息前都注入 `task_plan.md` 前 50 行和 `progress.md` 后 20 行。
- `UserPromptSubmit` 不注入完整 `findings.md`，只提醒 agent 在需要需求、设计、调研、约束、根因和决策时读取它。
